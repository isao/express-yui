...
function afterShift(file, next) {
	debug('afterShift file ' + file);
	next()
}

function Builder(options) {
    ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'Builder(options)', options);
...
Builder.prototype = {
    compile: function (mods) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'compile(mods)');
        this.process(mods);
        this.produceJSON();
        this.conditionals();
        this.produceJS();
    ...
    process: function (mods) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'process(mods)');
        var self = this;
    ...
    parseData: function (name, data, file) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'parseData(name, data, file), name =', name);
        var i, o;
        for (i in data) {
    ...
    conditionals: function () {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'conditionals()');
        var tests = [],
            allTests = [],
            jsonStr,
            self = this;
    ...
    produceJSON: function () {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'produceJSON()');
        this.data.json = this.sortObject(this.data.json);
    },
    produceJS: function () {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'produceJS()');
        this.data.js = this.header().replace(TEMPLATE_TOKEN, this.data.jsonStr);
    },
    header: function () {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'header()');
        var str = [
...
module.exports = {
    ...
    **/
    shiftFiles: function (files, options, callback) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'shiftFiles(files, options, callback)');
        ...
        function next() {
            ...
            if (file) {
                ...
                ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'next', 'spawn args', args, libpath.dirname(file));
                ...
                child.on('exit', function (code) {
                    ...
                    ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'shiftFiles{ child.on("exit") }');
                    ...
                    afterShift(file, next); // next item in queue to be processed
                });
    ...
    **/
    _checkBuildFile: function (file) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), '_checkBuildFile(file) file:', file);
        var mod,
            entry,
    ...
    **/
    _checkYUIModule: function (file) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), '_checkYUIModule(file)');
        ...
        contextForRunInContext.YUI = {
            add: function (name, fn, version, config) {
                ;console.log('~' + __filename.split('/').slice(-2).join('/'), 'contextForRunInContext.YUI.add(name, fn, version, config)');
                ;console.log('~  ', name);
    ...
    **/
    _isCached: function (file, buildDir) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), '_isCached(file, buildDir)');
        ;console.log('~  file %s, buildDir %s', file, buildDir);
        var fileHash,
            data;
    ...
    **/
    _clearCached: function (file, buildDir) {
        ;console.log('~' + __filename.split('/').slice(-2).join('/'), '_clearCached(file, buildDir)');
        ;console.log('~  file %s, buildDir %s', file, buildDir);
