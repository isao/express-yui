...
/**
 * Perform module directory hashing.
 * @param {array} filelist List of module files to hash and rename.
 * @param {string} buildDir Express-YUI build directory
 * @param {function} [cb] Callback on completion of shifting all files.
 */
function hashmodules(filelist, buildDir, cb) {
    var bldlist = [];
    filelist.forEach(function (file) {
        bldlist.push(libpath.join(buildDir, libpath.basename(file).replace(/\.[a-z]{2,4}$/, '')));
    });
    hashdirs(bldlist, cb || function() {});
}
    ...
    shiftFiles: function (files, options, callback) {
        ...
        function next() {
            ...
            if (file) {
                ...
                if (options.hashdirs) {
                    hashmodules(files, options.buildDir, callback);
                } else if (callback) {
                    callback(null);
                }
            }
