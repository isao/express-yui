...
function wereModulesRenamed(name, evt) {
    var files = Object.keys(evt.files),
        base = evt.bundle.baseDirectory,
        build = evt.bundle.yuiBuildDirectory,
        fullpath = path.join(build, name + '.json'),
        relpath = path.relative(base, fullpath); // "build/demo/loader-demo.json"

    if (evt.files[relpath]) {
        debug('NEED TO PROCESS NEW MODULE ' + relpath);
        debug('HERE IS THE RENAMING INFO');
        console.log(require(evt.files[relpath].fullPath));
        return true;
    }
}
    ...
    plugin: function (options) {
        ...
        return {
            ...
            bundleUpdated: function (evt, api) {
                ...
                if (wereModulesRenamed(moduleName, evt)) {
                    return;
                }
